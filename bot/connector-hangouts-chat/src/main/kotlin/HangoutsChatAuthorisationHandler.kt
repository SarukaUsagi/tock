...
class HangoutsChatAuthorisationHandler(private val botProjectNumber: String) : Handler<RoutingContext> {
    ...
    override fun handle(routingContext: RoutingContext) {
        val token = routingContext.request().getHeader("Authorization")?.takeIf { it.startsWith(BEARER_PREFIX) }?.substringAfter(BEARER_PREFIX)
    ...
    private fun hasFailure(token: String?): VerificationFailure? {
        val idToken: GoogleIdToken = GoogleIdToken.parse(jsonFactory,
